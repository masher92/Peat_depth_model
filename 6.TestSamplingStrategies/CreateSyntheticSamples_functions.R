...
"
  ...
  grid_shifts <- setNames(data.frame(matrix(ncol = 2, nrow = 0)), c("Long", "Lat"))
  ...
  for (i in (1:100000)){
    ...
    # If this position is not already in the list of positions, then add it
    if (nrow(match_df(grid_shifts, position, on = c("Long", "Lat")))==0){
      grid_shifts <- rbind(grid_shifts, position)} 
    # Once there are 50 rows in the dataframe, exit the loop
    if (nrow(grid_shifts) == 50) break}
  
  # Trim to be just 50 long
  grid_shifts <- grid_shifts[c(1:n_shifts),]
