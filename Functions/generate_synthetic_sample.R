...
RasterToDF_cropped <- function (aoi, raster) {
  'Clips a raster file to the extent of an AOI
  And converts to a dataframe.
  @param aoi_fp: 
  @param raster_fp: 
  @returns df:' 
  # Crop raster data by extent of sample area AOI.
...
select_depth_from_distrib_new <- function (samples_df, row_to_check, slope_elevation_category){
  "
Samples_df contains the slope and elevation of sampled depth points
slope_elevation_category is a category for which we are interested in assigning a reasonable depth value to.  
The function filters sample_df according to slope_elevation_category and takes a random sample from the depth values in the 
distribution.
"
  # Filter the samples_df to contain just those with the same slope/elevation category
  filtered_by_category <- samples_df[samples_df$slope_and_elevation == slope_elevation_category ,]
  
  # Find the distance between each of these and the synthetic sample
  # Location of sample 
  location <- c(row_to_check$longitude, row_to_check$latitude)
  
  smallest_distance <- 1000000
  for (i in (1:nrow(filtered_by_category))){
    dist <- distm(location, filtered_by_category[i,]@coords, fun = distHaversine)
  ...
  # Closest point
  closest_point <- filtered_by_category[closest_row,]
  
  # Assign the synthetic sample this depth
  row_to_check$depth <- closest_point$depth
  
  return(list(row_to_check, smallest_distance))}
